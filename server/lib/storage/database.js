  ...
  constructor(options = {}) {
    if (!options.provider) {
      throw new ArgumentError(
        "The 'provider' has to be set when initializing the database."
      );
    }
  ...
  canChange(type, checkFor, id) {
    return this.provider
      ...
      .then((items) =>
        _.filter(
          items,
          (item) => item[checkFor] && _.includes(item[checkFor], id)
        )
      )
      .then((items) => {
        if (items.length) {
    ...
    return this.getPermissions()
      .then((permissions) =>
        checkUnique(
          permissions.filter(
            (item) =>
              item.name.toLowerCase() === permission.name.toLowerCase() &&
              item.applicationId === permission.applicationId
          ),
        ...
        checkUnique(
          roles.filter(
            (item) =>
              ...
              item.applicationId === role.applicationId
          ),
